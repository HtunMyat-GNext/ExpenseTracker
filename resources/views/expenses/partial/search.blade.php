@if ($expenses->isNotEmpty())
        ...
        <tbody>
            ...
            @foreach ($expenses as $expense)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">
                        <img src="{{ $expense->image ? asset('images/' . $expense->image) : asset('images/dummy.jpeg') }}"
                            alt="" class="w-10 h-10 object-cover">
                    </td>
                    <td class="px-6 py-4">
                        {{ $expense->date ? $expense->date : date('Y-m-d', strtotime($expense->created_at)) }}
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex">
    ...
    <div class="m-4" id="paginate">
        {{ $expenses->appends(['search' => request()->query('search')])->links() }}
    </div>
@else
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        ...
        <tbody>
            <tr>
                <td class="bg-white dark:bg-gray-800 p-8 font-bold text-lg rounded-lg text-center" colspan="8">
                    <h3 class="dark:text-gray-400 text-gray-400 mx-auto">No expense found!</h3>
                </td>
            </tr>
        </tbody>
    </table>
